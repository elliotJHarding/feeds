 ...
 * of the last feed's - live only here.
 */
class ToggleFeedUseCase(
     ...
     * an end time is never before the feed's own start. Used by the widget and Quick
     * Settings tile, which have no UI to pick a time.
     */
    suspend fun toggle(sideOverride: Side? = null): Result {
        ...
        else start(sideOverride, Instant.now())
    }
